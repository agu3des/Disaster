...
-- =========================================================
-- SCRIPT PARA COMPLETAR BASE ATÉ A DISTRIBUIÇÃO SOLICITADA
-- - PostgreSQL
-- - Gera registros adicionais via generate_series (bulk)
-- - Preserva os 5 registros já inseridos em cada tabela conforme seu script inicial
-- =========================================================

-- 1) DESASTRE_NATURAL: queremos 100 total (já existem 5) => inserir codDesastre 6..100 (95 registros)
INSERT INTO Desastre_Natural (codDesastre, dataDeOcorrencia, duracao, intensidade, qtdVitimas)
SELECT s, (date '2023-01-01' + (s * interval '3 day'))::date,
       (12 + (s % 60)),            -- duracao (horas)
       ((s % 10) + 1),             -- intensidade 1..10
       (50 + ((s * 7) % 300))      -- qtdVitimas
FROM generate_series(6,100) AS gs(s);

-- 2) TESTEMUNHA: queremos 100 total (já existem 5) => inserir 6..100 (95 registros)
INSERT INTO Testemunha (testemunhaCpf, nome, genero, dataDeNascimento, nacionalidade, contato)
SELECT lpad((10000000000 + s)::text, 11, '0'),
       concat('Testemunha ', s),
       CASE WHEN (s % 2) = 0 THEN 'F' ELSE 'M' END,
       (date '1960-01-01' + ((s * 400) % 20000) * interval '1 day')::date,
       'Brasileira',
       lpad((11000000000 + s)::text, 11, '0')
FROM generate_series(6,100) AS gs(s);

-- 3) RELATO: queremos 100 total (já existem 5) => inserir 6..100 (95 registros)
-- apontando para codDesastre = s (6..100) e testemunhas recém-criadas (cpf correspondente)
INSERT INTO Relato (codigoRelato, relato, dataDeEmissao, codDesastre, testemunha)
SELECT s,
       concat('Relato auto-gerado número ', s, '. Observações: danos parciais e necessidade de auxílio.'),
       (date '2023-01-02' + (s * interval '3 day'))::date,
       s,  -- referencia desastre 6..100
       lpad((10000000000 + s)::text, 11, '0')
FROM generate_series(6,100) AS gs(s);

-- 4) TIPO: já tem 5 tipos no seu script, não adicionamos (manter 1..5).

-- 5) ABRIGO: queremos 50 total (já existem 5) => inserir cnpj 6..50 (45 registros)
INSERT INTO Abrigo (cnpj, nome, cep, numero, rua, bairro, cidade, estado, disponibilidade)
SELECT lpad((10000000000000 + s)::text, 14, '0'),
       concat('Abrigo ', s),
       lpad(((10000000 + (s * 37)) % 99999999)::text, 8, '0'),
       (s * 10) % 999,
       concat('Rua ', chr(65 + (s % 26))),
       concat('Bairro ', (s % 20) + 1),
       CASE (s % 8)
         WHEN 0 THEN 'São Paulo' WHEN 1 THEN 'Rio de Janeiro' WHEN 2 THEN 'Belo Horizonte'
         WHEN 3 THEN 'Porto Alegre' WHEN 4 THEN 'Fortaleza' WHEN 5 THEN 'Curitiba'
         WHEN 6 THEN 'Salvador' WHEN 7 THEN 'Recife' ELSE 'CidadeX' END,
       CASE (s % 5) WHEN 0 THEN 'SP' WHEN 1 THEN 'RJ' WHEN 2 THEN 'MG' WHEN 3 THEN 'RS' WHEN 4 THEN 'CE' END,
       (s % 2 = 0)
FROM generate_series(6,50) AS gs(s);


-- 6) VITIMA: queremos 200 total (já existem 5) => inserir 6..200 (195 registros)
INSERT INTO Vitima (vitimaCpf, nome, dataDeNascimento, genero, nacionalidade, condicaoMedica, cnpjAbrigo, dataEntrada, dataSaida, contato)
SELECT lpad((30000000000 + s)::text, 11, '0'),
       concat('Vitima ', s),
       (date '1970-01-01' + ((s * 365) % 18000) * interval '1 day')::date,
       CASE WHEN (s % 2)=0 THEN 'F' ELSE 'M' END,
       'Brasileiro',
       CASE (s % 5)
         WHEN 0 THEN 'Ferido' WHEN 1 THEN 'Leve' WHEN 2 THEN 'Grave' WHEN 3 THEN 'Em observação' WHEN 4 THEN 'Sem ferimentos' END,
       -- atribui abrigo de forma circular entre 1..50 (existentes 1..5 + gerados 6..50)
       lpad((10000000000000 + ((s % 50) + 6))::text, 14, '0'),
       (date '2023-01-01' + ((s % 400) * interval '1 day'))::date,
       NULL,
       lpad((13000000000 + s)::text, 11, '0')
FROM generate_series(6,200) AS gs(s);

-- 7) CATEGORIA: queremos 10 total (já existem 5) => inserir codCateg 6..10 (5 registros)
INSERT INTO Categoria (codCateg, descricao, valor, perecivel, validade)
SELECT s,
       concat('Categoria ', s),
       (100 + (s * 10))::numeric(20,2),
       CASE WHEN (s % 2)=0 THEN true ELSE false END,
       CASE WHEN (s % 2)=0 THEN (date '2024-12-31') ELSE NULL END
FROM generate_series(6,10) AS gs(s);

-- 8) DOADOR: queremos 100 total (já existem 5) => inserir 6..100 (95 registros)
INSERT INTO Doador (doadorCpf, nome)
SELECT lpad((40000000000 + s)::text, 11, '0'),
       concat('Doador ', s)
FROM generate_series(6,100) AS gs(s);

-- 9) DOACAO: queremos 100 total (já existem 5) => inserir codDoacao 6..100 (95 registros)
-- codCateg 1..10 referenciando categorias existentes; doadorCPF referenciando doadores criados
INSERT INTO Doacao (codDoacao, qtdEstoque, dataDoacao, localArmazenamento, codCateg, doadorCPF)
SELECT s,
       (10 + (s % 200)),
       (date '2023-01-10' + ((s * 5) * interval '1 day'))::date,
       concat('Depósito ', (s % 10) + 1),
       ((s % 10) + 1),  -- codCateg 1..10
       -- map doador cpf (existing ones 12345678901 etc exist; we generated doadores starting at padded 40000000006)
       CASE WHEN s <= 5 THEN lpad('12345678901',11,'0') ELSE lpad((40000000000 + s)::text, 11, '0') END
FROM generate_series(6,100) AS gs(s);

-- 10) AGENCIADERESCATE: queremos 20 total (já existem 5) => inserir cnpj 6..20 (15 registros)
INSERT INTO AgenciaDeResgate (cnpj, nomeAgencia, telefone, cep, numero, rua, bairro, cidade, estado)
SELECT lpad((50000000000000 + s)::text, 14, '0'),
       concat('Agência ', s),
       lpad((14000000000 + s)::text, 10, '0'),
       lpad(((20000000 + s * 11) % 99999999)::text, 8, '0'),
       (s * 3) % 999,
       concat('Rua A', s % 50),
       concat('Bairro ', (s % 25) + 1),
       CASE (s % 6) WHEN 0 THEN 'São Paulo' WHEN 1 THEN 'Rio de Janeiro' WHEN 2 THEN 'Curitiba' WHEN 3 THEN 'Fortaleza' WHEN 4 THEN 'Porto Alegre' ELSE 'Belo Horizonte' END,
       CASE (s % 5) WHEN 0 THEN 'SP' WHEN 1 THEN 'RJ' WHEN 2 THEN 'PR' WHEN 3 THEN 'CE' WHEN 4 THEN 'RS' END
FROM generate_series(6,20) AS gs(s);

-- 11) VOLUNTARIO: queremos 100 total (já existem 5) => inserir 6..100 (95 registros)
INSERT INTO Voluntario (cpfVoluntario, nomeVoluntario)
SELECT lpad((60000000000 + s)::text, 11, '0'),
       concat('Voluntario ', s)
FROM generate_series(6,100) AS gs(s);


-- =========================================================
-- Agora os RELACIONAMENTOS (para somar +70 inserts e completar 1000 linhas no total)
-- Distribuição de inserts em cada tabela de relacionamento conforme cálculo:
-- voluntarioAbrigo +9, voluntarioAgencia +9, ocorrencia +9, TipoDesastre +9,
-- VitimaDesastre +9, DoacaoVitima +9, DoacaoAgencia +8, atuacao +8
-- =========================================================

-- 12) voluntarioAbrigo: inserir 9 * 8? vamos gerar 9 entradas por s de 6..14 (9 valores) -> 9*? 
-- Simples: gerar pares únicos combinando parte das ranges (gera 9*? = 9*?).
-- Implementação: gera 9 * ((6..14)) -> 9 registros (s 6..14) cada com um volunteer mapped to an abrigo
INSERT INTO voluntarioAbrigo (cnpjAbrigo, CPFVoluntario)
SELECT lpad((20000000000000 + ((s % 50) + 1))::text, 14, '0'),
       lpad((60000000000 + s)::text, 11, '0')
FROM generate_series(6,14) AS gs(s);

-- 13) voluntarioAgencia: 9 inserts (6..14) mapping volunteers to agencies
INSERT INTO voluntarioAgencia (cnpjAgencia, CPFVoluntario)
SELECT lpad((50000000000000 + ((s % 20) + 1))::text, 14, '0'),
       lpad((60000000000 + (s + 10))::text, 11, '0')
FROM generate_series(6,14) AS gs(s);

-- 14) ocorrencia: 9 inserts (codDesastre 6..14 mapped to regiones 1..5)
INSERT INTO ocorrencia (codigoRegiao, codDesastre)
SELECT ((s % 5) + 1), s
FROM generate_series(6,14) AS gs(s);

-- 15) TipoDesastre: 9 inserts (map each new desastre 6..14 to a tipo)
INSERT INTO TipoDesastre (codTipo, codDesastre)
SELECT ((s % 5) + 1), s
FROM generate_series(6,14) AS gs(s);

-- 16) VitimaDesastre: 9 inserts - cada um mapeia uma vitima a um desastre
-- usando vitimaCpf from the vitimas created (6..14) mapped to disasters 6..14
INSERT INTO VitimaDesastre (codDesastre, vitimaCpf)
SELECT s, lpad((30000000000 + s)::text, 11, '0')
FROM generate_series(6,14) AS gs(s);

-- 17) DoacaoVitima: 9 inserts - map doacoes 6..14 to vitimas 6..14
INSERT INTO DoacaoVitima (codDoacao, vitimaCpf)
SELECT s, lpad((30000000000 + s)::text, 11, '0')
FROM generate_series(6,14) AS gs(s);

-- 18) DoacaoAgencia: 8 inserts (15..22) map some doacoes to agencias
-- We'll use codDoacao 6..13 to map to agencias 6..13
INSERT INTO DoacaoAgencia (codDoacao, cnpjAgencia)
SELECT s, lpad((50000000000000 + ((s % 20) + 1))::text, 14, '0')
FROM generate_series(6,13) AS gs(s);

-- 19) atuacao: 8 inserts map agencias 6..13 to desastres 6..13 with dates
INSERT INTO atuacao (cnpjAgencia, codDesastre, dataDeAtuacao)
SELECT lpad((50000000000000 + ((s % 20) + 1))::text, 14, '0'),
       s,
       (date '2024-01-01' + (s * interval '2 day'))::date
FROM generate_series(6,13) AS gs(s);
...
-- Agências de Resgate
SELECT * FROM AgenciaDeResgate;

-- Voluntários
SELECT * FROM Voluntario;

-- Ocorrências (Região ↔ Desastre)
SELECT * FROM Ocorrencia;

-- TipoDesastre (Tipo ↔ Desastre)
SELECT * FROM TipoDesastre;

-- VitimaDesastre (Vítima ↔ Desastre)
SELECT * FROM VitimaDesastre;

-- DoacaoVitima (Doação ↔ Vítima)
SELECT * FROM DoacaoVitima;

-- DoacaoAgencia (Doação ↔ Agência)
SELECT * FROM DoacaoAgencia;

-- Atuação (Agência ↔ Desastre)
SELECT * FROM Atuacao;

-- Voluntário ↔ Agência
SELECT * FROM VoluntarioAgencia;

-- Voluntário ↔ Abrigo
SELECT * FROM VoluntarioAbrigo;
